      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <motion.div 
          ...
          className={`mt-6 bg-gradient-to-r ${
            isLogin ? 'from-blue-50 to-purple-50 border-blue-200' : 'from-purple-50 to-blue-50 border-purple-200'
          } rounded-2xl p-6 border`}
        >
          <div className="text-center">
            <div className="text-2xl mb-2">{isLogin ? '🎯' : '🎁'}</div>
            <p className={`text-sm font-medium ${isLogin ? 'text-blue-700' : 'text-purple-700'}`}>
              {isLogin ? (
                <>
                  <strong>Demo Login:</strong> Use any email format and password (minimum 6 characters)
                </>
              ) : (
                <>
                  <strong>Welcome Bonus:</strong> Get ₹50 cashback on your first recharge!
                </>
              )}
            </p>
            {!isLogin && (
              <p className="text-xs text-purple-600 mt-1">
                Plus exclusive offers and instant recharge benefits
              </p>
            )}
          </div>
        </motion.div>
